...
Stack_Size      EQU     0x00000400 ;//定义1KB大小的栈空间
                ...
                AREA    STACK, NOINIT, READWRITE, ALIGN=3 ;//定义一个段,8(2的3次方)字节对齐 的空间作为栈
...
; Vector Table Mapped to Address 0 at Reset ;//开始使用thumb指令模式
...
__Vectors_Size  EQU  __Vectors_End - __Vectors ;//计算 中断向量表长度
...
Reset_Handler   PROC
                ...
                LDR     R0, =SystemInit
                BLX     R0               
                LDR     R0, =__main	; //__main()是编译系统提供的一个函数，负责完成库函数的初始化和初始化应用程序执行环境，最后自动跳转到main()
                BX      R0
                ENDP
