...
void dma1_ch5_peri2mem_start(unsigned char *src, unsigned char *dst, unsigned int len)
{
	...
	DMA_ITConfig(DMA1_Channel5,DMA_IT_TE,ENABLE); /* 错误中断*/
...
static void dma1_ch5_nvic_config(void)
{
	...
	DMA_ITConfig(DMA1_Channel5,DMA_IT_HT,DISABLE); /* 半传输中断  */
	DMA_ITConfig(DMA1_Channel5,DMA_IT_TC,DISABLE); /* 传输完成中断*/
	DMA_ITConfig(DMA1_Channel5,DMA_IT_TE,DISABLE); /* 错误中断*/
}
